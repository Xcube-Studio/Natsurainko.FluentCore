...
namespace Natsurainko.FluentCore.Module.Parser
{
    public class GameCoreParser
    {
        ...
        private IEnumerable<LibraryResource> CheckRepeat(IEnumerable<LibraryResource> libraries)
        {
            var raw = libraries.ToList();
            var same = new List<(LibraryResource, LibraryResource)>();
            ...
            foreach (var item in same)
            {
                try
                {
                    var v1 = new Version(item.Item1.Name.Split(':')[2]);
                    var v2 = new Version(item.Item2.Name.Split(':')[2]);

                    if (v1 >= v2)
                        remove.Add(item.Item2);
                    else remove.Add(item.Item1);
                }
                catch { }
            }
