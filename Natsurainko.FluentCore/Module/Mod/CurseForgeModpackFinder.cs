    ...
    public class CurseForgeModpackFinder
    {
        ...
        public async Task<List<CurseForgeModpack>> GetModpacksAsync(int[] modIds)
        {
            ...
            try
            {
                ...
                foreach (JObject jObject in (JArray)entity["data"])
        ...
        public async Task<List<CurseForgeModpack>> GetFeaturedModpacksAsync()
        {
            ...
            try
            {
                ...
                foreach (JObject jObject in (JArray)entity["data"]["popular"])
                    result.Add(ParseCurseForgeModpack(jObject));

                foreach (JObject jObject in (JArray)entity["data"]["recentlyUpdated"])
                    result.Add(ParseCurseForgeModpack(jObject));

                foreach (JObject jObject in (JArray)entity["data"]["featured"])
                    result.Add(ParseCurseForgeModpack(jObject));
