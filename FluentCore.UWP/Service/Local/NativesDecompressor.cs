...
namespace FluentCore.UWP.Service.Local
{
    public class NativesDecompressor
    {
        ...
        {
            nativesFolder = string.IsNullOrEmpty(nativesFolder) ? $"{PathHelper.GetVersionFolder(Root, Id)}\\natives" : nativesFolder;

            if (!Directory.Exists(nativesFolder))
                Directory.CreateDirectory(nativesFolder);

            foreach (var item in natives)
                using (ZipArchive zip = ZipFile.OpenRead($"{PathHelper.GetLibrariesFolder(Root)}\\{item.GetRelativePath()}"))
