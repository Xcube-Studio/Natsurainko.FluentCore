using FluentCore.UWP.Exceptions.Launcher;
using FluentCore.UWP.Interface;
using FluentCore.UWP.Model.Launch;
using FluentCore.UWP.Service.Component.Launch;
    ...
    public class MinecraftLauncher : ILauncher
    {
        ...
        public virtual void Launch(string id)
        {
            ...
            if (string.IsNullOrEmpty(this.LaunchConfig.NativesFolder))
                this.LaunchConfig.NativesFolder = $"{PathHelper.GetVersionFolder(core.Root, id)}\\natives";
        ...
        /// </summary>
        /// <returns></returns>
        public virtual Task<LaunchResult> WaitForResult()
        {
            return Task.Run(delegate
            {
                ...
                ProcessContainer.Process.WaitForExit();
                return new LaunchResult
                {
                    Args = this.ProcessContainer.Process.StartInfo.Arguments,
                    Errors = this.ProcessContainer.ErrorData,
